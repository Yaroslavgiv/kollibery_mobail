...
class _OrderListScreenState extends State<OrderListScreen>
  ...
  Widget _getStatusIcon(String status) {
    switch (status.toLowerCase()) {
      ...
      case 'processing':
        return Icon(Icons.work, size: 16, color: Colors.blue);
      case 'preparing':
        return Icon(Icons.inventory_2, size: 16, color: Colors.blue.shade700);
      case 'in_transit':
        return Icon(Icons.local_shipping, size: 16, color: Colors.green);
      case 'shipped':
        return Icon(Icons.local_shipping, size: 16, color: Colors.green);
      case 'delivered':
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          // Если заказ доставлен, можно добавить кнопку для отслеживания
          if (order.status.toLowerCase() == 'delivered')
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                // Можно добавить переход к экрану отслеживания
              },
