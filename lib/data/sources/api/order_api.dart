...
class OrderApi {
  ...
  static Future<bool> placeOrder({
    ...
    try {
      ...
      print('   Бэкенд должен определить продавца по productId и сохранить связь заказа с продавцом');
      ...
      if (productSellerId != null) {
        ...
        print('   2. Определить sellerId из product.userId (товар не найден в локальной карте)');
      }
      print('   3. Сохранить заказ с связью к продавцу в БД');
      print('   4. При запросе GET /order/getorders?role=seller вернуть заказ для этого продавца');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (productSellerId != null) {
          ...
          print('   1. Определил sellerId=$productSellerId по productId=$productId');
          print('   2. Сохранил связь заказа с продавцом в БД');
          print('   3. При запросе /order/getorders?role=seller вернет этот заказ для продавца $productSellerId');
        } else {
          print('⚠️ ВНИМАНИЕ: Не удалось определить sellerId из продукта!');
          print('   Бэкенд должен сам определить продавца по productId=$productId');
        }
      } else {
        ...
        try {
          ...
          if (responseBody.isNotEmpty) {
            ...
            if (responseData is Map) {
              ...
              final responseSellerId = responseData['sellerId'] ?? responseData['seller_id'];
              ...
              if (responseSellerId != null) {
                ...
                if (productSellerId != null) {
                  if (responseSellerId.toString() == productSellerId.toString()) {
                    print('✅ sellerId из ответа совпадает с sellerId из продукта');
                  } else {
                    print('⚠️ ВНИМАНИЕ: sellerId из ответа ($responseSellerId) не совпадает с sellerId из продукта ($productSellerId)');
                  }
                }
              } else {
                print('⚠️ Бэкенд НЕ вернул sellerId в ответе');
                print('💡 Проверьте, что бэкенд определяет sellerId по productId и сохраняет связь');
              }
            } else {
  ...
  static Future<List<Map<String, dynamic>>> fetchOrdersByRole(
    ...
    try {
      ...
      if (role == 'seller') {
        ...
        print('💡 И вернуть только заказы, где товар принадлежит этому продавцу');
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (role == 'seller') {
          ...
          if (ordersData.isEmpty) {
            ...
            print('      2. Бэкенд не связывает заказ с продавцом при создании');
            print('      3. Фильтрация по роли "seller" работает неправильно');
            print('      4. Токен не содержит правильную информацию о продавце');
          } else {
            print('   📋 Список заказов с сервера:');
            for (var i = 0; i < ordersData.length; i++) {
              final order = ordersData[i] as Map<String, dynamic>;
              print('      ${i + 1}. Заказ ID: ${order['id']}, productId: ${order['productId']}');
            }
          }
        }
        ...
        // Загружаем все товары один раз и строим карту по id
        final productsMap = await _fetchAllProductsMap();
        print('📦 Загружено ${productsMap.length} товаров для обогащения заказов');
        ...
        for (final orderData in ordersData) {
          ...
          if (productId != null && productsMap.containsKey(productId)) {
            ...
            if (role == 'seller') {
              ...
              if (productSellerId != null && currentUserId != null) {
                ...
                if (currentUserIdStr == productSellerIdStr) {
                  ...
                  print('     ⚠️ ВНИМАНИЕ: Товар принадлежит другому продавцу!');
                  print('        - Текущий продавец: "$currentUserIdStr"');
                  print('        - Продавец товара: "$productSellerIdStr"');
                  print('     💡 ВАЖНО: Бэкенд должен фильтровать заказы, включаем заказ для проверки');
                }
              } else {
            ...
            if (role == 'seller') {
              print('     ⚠️ Товар не найден в локальной базе, но включаем заказ');
              print('     💡 Бэкенд должен фильтровать заказы по продавцу');
              print('     💡 Если заказ отображается, значит бэкенд вернул его для текущего продавца');
            }
            enrichedOrders.add(order);
          }
        }
        ...
        if (role == 'seller') {
          ...
          print('   - Заказов после обогащения данными: ${enrichedOrders.length}');
          if (enrichedOrders.isEmpty && ordersData.isNotEmpty) {
            print('   ⚠️ ВНИМАНИЕ: Заказы получены с сервера, но не обогащены данными!');
            ...
            print('   ✅ Получено ${enrichedOrders.length} заказов для отображения');
