...
class OrderRepository {
  ...
  Future<List<OrderModel>> fetchSellerOrdersAsModels() async {
    ...
    print('📦 Получено ${ordersData.length} заказов для продавца (после фильтрации на API)');
    final orders = ordersData.map((data) => OrderModel.fromJson(data)).toList();
    // НЕ применяем локально сохраненные статусы для продавца - используем статусы с сервера
    print('✅ Возвращено ${orders.length} заказов для продавца (без применения локальных статусов)');
    return orders;
  }
